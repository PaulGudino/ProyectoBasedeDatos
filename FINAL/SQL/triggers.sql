...
AND NEW.USUARIO_ADMIN = USUARIO_ADMIN);
...
/*TRIGGER QUE HACE QUE NO SE REPITA EL INGRESO DEL MISMO USUARIO EN EL MISMO MES */
DELIMITER ||
CREATE TRIGGER VALIDACION_SI_YA_EXISTE_EL_INGRESO BEFORE INSERT ON INGRESOS FOR EACH ROW
BEGIN
SET @VALIDACION = (SELECT ID_INGRESOS FROM INGRESOS WHERE MONTH(NEW.FECHA_INGRESO) = MONTH(NEW.FECHA_INGRESO) 
AND NEW.USUARIO_ADMIN = USUARIO_ADMIN LIMIT 1);
IF(@VALIDACION IS NOT NULL) THEN
SIGNAL SQLSTATE "45000" SET MESSAGE_TEXT = "EL INGRESO YA ESTA INGRESADO EN EL MES CORRESPONDIENTE";
END IF;
END ||
DELIMITER ;
