...
END ||
DELIITER ;
SELECT MENSAJE,FECHA FROM BUZON, ADMINISTRADOR WHERE USUARIO = USUARIO_ADMIN AND ID_FAMILIA = NUMERO;

/*PROCEDURE QUE MUESTRA EL VALOR TOTAL POR CATEGORIA DE CADA FAMILIA DEPENDIENDO DEL AÑO*/
DELIMITER ||
CREATE PROCEDURE CATEGORIA_AÑO (IN ANIO INT)
BEGIN
...
GROUP BY AÑO ,ID_FAMILIA HAVING AÑO = ANIO;
END ||
DELIMITER ;

/*PROCEDURE QUE MUESTRA EL TOTAL DE GASTOS E INGRESOS POR FAMILIA DE ACUERDO A UN AÑO */
DELIMITER ||
CREATE PROCEDURE GASTOS_INGRESOS_AÑO (IN ANIO INT)
BEGIN
IF (@VALIDACION IS NULL OR ANIO IS NULL) THEN
SIGNAL SQLSTATE "45000" SET MESSAGE_TEXT = "NO EXISTE EL AÑO";
END IF;
...
/*PROCEDURE QUE MUESTRA CUAL ES MAYOR GASTO REGISTRADO POR DISTRACCION DE CADA FAMILIA EN LA TABLA HISTORIALFINANZAS*/
DELIMITER ||
CREATE PROCEDURE MAYOR_GASTO_CATEGORIA_DISTRACCION ()
BEGIN
SELECT AÑO, MES, DISTRACCION, HF.ID_FAMILIA
FROM HISTORIALFINANZAS HF, (SELECT MAX(DISTRACCION) AS MAXIMO, ID_FAMILIA FROM HISTORIALFINANZAS GROUP BY ID_FAMILIA) AS T
...
/*PROCEDURE QUE MUESTRA LOS MENSAJE QUE LE PERTENECEN AL ADMINISTRADOR*/
DELIMITER ||
...
-- DELETE : ELIMINA DATOS
DELIMITER ||
CREATE PROCEDURE ELIMINAR_GASTOS(IN NOMBRE_N VARCHAR(100))
BEGIN
	DELETE FROM  GASTOS WHERE NOMBRE = NOMBRE_N;
END
DELIMITER ;
